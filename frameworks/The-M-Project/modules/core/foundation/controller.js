...
/** @scope M.Controller.prototype */ {
    ...
    switchToTab: function(tab) {
        ...
        // 3/26/13: NEW var to determine if can switch tabs even if currentPage === newPage
        var alwaysSwitch = tab.parentView.alwaysSwitch;

        /* store the active tab in tab bar view */
        tab.parentView.setActiveTab(tab);

        // 3/26/13: changed conditional because it was preventing from switching to correct page
        if(tab === currentTab) {
            var currentPage = M.ViewManager.getCurrentPage();
            // 3/25/13: NEW conditional - can choose if want to always switch
            if(alwaysSwitch || currentPage !== newPage) {
