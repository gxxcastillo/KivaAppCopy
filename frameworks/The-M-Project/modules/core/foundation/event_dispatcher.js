    ...
    registerEvent: function(type, eventSource, handler, recommendedEvents, sourceType, isInternalCall, skipUnbinding) {
	    ...
	    if( typeof mixpanel != 'undefined'){
		    if (event.type == 'click' || event.type == 'tap') {
			    ...
			    trackData = {
				    'environment': M.Application.getConfig('environment')
					    ,'version': M.Application.getConfig('version')
					    ,'currentPage': M.ViewManager.getCurrentPage().value
					    ,'platform': platform
			    };

			    mixpanel.track('click', trackData);
		    }
	    }
